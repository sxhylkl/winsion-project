    ...
    <el-table :data="tableData" border style="width: 100%" height="680">
      <el-table-column label="编号" width="200" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.classificationNo }}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleEdit(data) {
        this.$store.dispatch(types.CONTENT4_EDIT_INFO_ACTION,data)
      },
      handleDelete(data) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          this.$store.dispatch(types.CONTENT4_CONFIRM_DELETE_INFO_ACTION,{data:data,_this:this})
        }).catch(()=>{})
      }
    }
  };
